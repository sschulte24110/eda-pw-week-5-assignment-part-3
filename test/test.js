    ...
    describe('STRETCH: `search` takes in a criteria object and returns an array of matching albums', function () {
      it('STRETCH: `search` takes in a criteria object and returns an array of matching albums', function () {
        ...
        if (search === undefined) {
          ...
          expect(search(primesCollection, {artist: 'Wilco', year: 1908}), 'Expect an empty array when searching for a Wilco album from 1908').to.be.an('array').that.is.empty;
          expect(search(primesCollection, {artist: 'Wilco', year: 2002})).to.deep.equal(wilcoResults);
          expect(search(primesCollection, {artist: 'Hiroshi Yoshimura', year: 1986})).to.deep.equal(hiroshiResults);
        }
      });
    });
});
